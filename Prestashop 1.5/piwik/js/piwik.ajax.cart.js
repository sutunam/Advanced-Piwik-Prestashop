...
var ajaxCart = {
	...
	updateCart : function(jsonData) {
		...
		else
		{
			...
			/**
			* Here it is important to add all other products found in the
			* cart, even the products not updated by the current "Add to
			* cart" click
			*/
			if (typeof Piwik !== 'undefined'){
				...
				$.each(jsonData.products, function(){
					piwikTracker.addEcommerceItem(
						this.id, // (required) SKU: Product unique identifier
						this.name, // (optional) Product name
						null, // (optional) Product category, string or array of up to 5 categories
						ps_round(parseFloat(this.price.replace(/[^0-9\.]/g,'')) / conversion_rate, 2), // (recommended) Product price
						this.quantity // (optional, default to 1) Product quantity
					);
				});

				// Records the cart for this visit
				var cart_total = parseFloat(jsonData.total.replace(/[^0-9\.]/g,''));
				piwikTracker.trackEcommerceCartUpdate(
					ps_round(cart_total / conversion_rate, 2) // (recommended) Cart amount
				);
			}
		}
	},
