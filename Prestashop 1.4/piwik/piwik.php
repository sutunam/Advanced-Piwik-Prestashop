...
class piwik extends Module {
	...
	public function _displayForm() {
		...
		$smarty->assign('default_currency', $currency->name . ' (' . $currency->sign . ')');
	...
	public function hookFooter($params) {
		...
		if ($page_name == 'order' AND Configuration::get('PIWIK_ORDER_TRACKING_METHOD') == 'php'){
		...
		return $this->display(__FILE__, 'js.tracking.all.tpl');
	}
	...
	private function _insertProductTracker($product){
		...
		$cleaned_product['id'] = intval($product->id);
		$cleaned_product['name'] = addslashes($product->name);
	...
	public function hookHeader($params){
		...
		if (in_array('cart', $this->_tracking_types)){
			...
			$smarty->assign('CUSTOMIZE_TEXTFIELD', _CUSTOMIZE_TEXTFIELD_);
			$smarty->assign('conversion_rate', $conversion_rate);
...
//		echo Configuration::get('PIWIK_TOKEN');die();
	...
	private function _fetchOrderInfo($order){
		...
		foreach ($products AS $product){
			...
			foreach($categories as $category){
				$cleaned_product['Category'] .= '"' . addslashes($category['name']) . '",';
				$cleaned_product['Category_arr'][] = $category['name']; // for use with PHP tracking
			}
			$cleaned_product['Category'] = rtrim($cleaned_product['Category'], ',');
			$cleaned_product['Category'] .= ']';
	...
	private function _getIPAddress($id_customer, $order_date_add){
		...
		$connections = $customer->getConnections();
		foreach($connections as $connection){
			$connection_date = date('Y-m-d', strtotime($connection['date_add']));
			if (strcmp($order_date_add, $connection_date) === 0){
				$ip_address = long2ip($connection['ip_address']);
				break;
			}
		}

		if (!isset($ip_address)){
			$ip_address = long2ip($connections[0]['ip_address']);
		}
